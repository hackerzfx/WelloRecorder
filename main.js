...
function findXVastFromRegistry() {
  try {
    ...
    for (const query of queries) {
      try {
        // Run synchronously to keep it simple in the loop
        const stdout = execSync(query, { stdio: 'pipe' }).toString();
        // Parse output: (Default)    REG_SZ    C:\Path\To\xvast.exe
...
ipcMain.handle('detect-browsers', async () => {
  ...
  for (const [name, paths] of Object.entries(BROWSER_PATHS)) {
    ...
    if (!found && name === 'xvast') {
      const regPath = findXVastFromRegistry();
      if (regPath) {
...
ipcMain.on('launch-browser', (event, browserPath) => {
  ...
  if (isFirefox) {
    ...
    let flags = ` --disable-gpu --disable-software-rasterizer --disable-gpu-compositing --disable-gpu-rasterization --disable-gpu-sandbox --disable-accelerated-2d-canvas --disable-accelerated-video-decode --disable-accelerated-mjpeg-decode --disable-d3d11 --user-data-dir="${safeProfileDir}" --no-first-run --no-default-browser-check --new-window`;

    if (isXVast) {
      // Add Widevine/DRM bypass flags for XVast
      flags += ` --disable-features=HardwareMediaKeyHandling --disable-gpu-driver-bug-workarounds`;
