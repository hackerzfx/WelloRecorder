...
void Inject(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (args.Length() < 2 || !args[0]->IsString() || !args[1]->IsString()) {
        isolate->ThrowException(Exception::TypeError(
            String::NewFromUtf8(isolate, "Expected (processName: string, dllPath: string)").ToLocalChecked()));
        return;
    }

    // Get process name
    v8::String::Value processNameValue(isolate, args[0]);
    std::wstring processName(reinterpret_cast<const wchar_t*>(*processNameValue));

    // Get DLL path
    v8::String::Value dllPathValue(isolate, args[1]);
    std::wstring dllPath(reinterpret_cast<const wchar_t*>(*dllPathValue));

    // Find the process
    DWORD pid = FindProcessId(processName);
    if (pid == 0) {
        isolate->ThrowException(Exception::Error(
            String::NewFromUtf8(isolate, "Process not found").ToLocalChecked()));
        return;
    }

    // Inject the DLL using Manual Map
    bool success = ManualMap(pid, dllPath);
