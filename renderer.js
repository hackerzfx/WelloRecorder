...
let sourcesPromise = null; // Optimization: Pre-fetch sources
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    startBtn.addEventListener('click', () => {
        ...
        // OPTIMIZATION: Start fetching sources immediately during countdown
        if (selectedMode !== 'camera') {
            sourcesPromise = ipcRenderer.invoke('get-sources');
        }
...
async function startRecording() {
    try {
        ...
        if (selectedMode === 'camera') {
            ...
            // Use pre-fetched promise or fetch now if missing
            const inputSources = await (sourcesPromise || ipcRenderer.invoke('get-sources'));
